...
---pause_hotkey  = obs.OBS_INVALID_HOTKEY_ID
...
function set_time_text()
	...
	local focus_minutes       = math.floor(total_minutes % 60)
	...
	local text          = string.format("Focus %02d:%02d\npomodoro %d/%d", focus_minutes, seconds, curr_pomo, total_pomo)
	...
	if cur_seconds < 1 then
		...
		-- else
			-- text = "stop_text"
			-- curr_pomo = curr_pomo + 1
		text = string.format("%s\npomodoro %d/%d", stop_text, curr_pomo, total_pomo)
		-- end
	end
...
function script_properties()
	...
	obs.obs_properties_add_int(props, "focus_minutes", "Focus Minutes", 0, 59, 1)
	obs.obs_properties_add_int(props, "seconds", "Focus Seconds", 0, 59, 1)
...
function script_description()
	...
	return "Timer Source counts down to 00:00 and displays how many pomodoros have been completed during this session. To reset the number of pomodoros, right click on the script and click \"Reload.\""
end
...
function script_update(settings)
	...
	total_seconds = (obs.obs_data_get_int(settings, "days")*24*60*60) + (obs.obs_data_get_int(settings, "hours")*60*60) + (obs.obs_data_get_int(settings, "focus_minutes")*60) + obs.obs_data_get_int(settings, "seconds")
...
function script_defaults(settings)
	...
	obs.obs_data_set_default_int(settings, "focus_minutes", 0)
